import { Table, MigrationInterface, QueryRunner, TableForeignKey, TableIndex } from "typeorm";
...
class Init20200214205800 implements MigrationInterface {
  public up = async (runner: QueryRunner) => {
    ...
    await runner.createIndex(tableNames.USERS, new TableIndex({
    ...
    await runner.createIndex(tableNames.POSTS, new TableIndex({
      columnNames: ['feed_id'],
    }));
    await runner.createIndex(tableNames.POSTS, new TableIndex({
    ...
    await runner.createPrimaryKey(tableNames.USER_FEED_RELATIONS, ['user_id', 'feed_id']);
    ...
    await runner.createPrimaryKey(tableNames.POST_MEDIA_RELATIONS, ['media_id', 'post_id']);
    ...
    await runner.createIndex(tableNames.COMMENTS, new TableIndex({
      columnNames: ['post_id'],
    }));

    await runner.createForeignKey(tableNames.MEDIA, new TableForeignKey({
    ...
    await runner.createForeignKey(tableNames.POSTS, new TableForeignKey({
      columnNames: ['creator_id'],
      referencedColumnNames: ['id'],
      referencedTableName: tableNames.USERS,
    }));
    ...
    await runner.createForeignKey(tableNames.USER_FEED_RELATIONS, new TableForeignKey({
      columnNames: ['user_id'],
      referencedColumnNames: ['id'],
      referencedTableName: tableNames.USERS,
    }));
    ...
    await runner.createForeignKey(tableNames.COMMENTS, new TableForeignKey({
      columnNames: ['creator_id'],
      referencedColumnNames: ['id'],
      referencedTableName: tableNames.USERS,
    }));
    ...
    await runner.createForeignKey(tableNames.POST_MEDIA_RELATIONS, new TableForeignKey({
